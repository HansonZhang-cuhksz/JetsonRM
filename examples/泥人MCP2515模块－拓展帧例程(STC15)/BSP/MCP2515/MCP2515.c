...
void MCP2515_Init(void)
{
	...
	MCP2515_WriteByte(TXB0SIDL,0xEB);//发送缓冲器0标准标识符低位(第3位为发送拓展标识符使能位)
	MCP2515_WriteByte(TXB0EID8,0xFF);//发送缓冲器0拓展标识符高位
	MCP2515_WriteByte(TXB0EID0,0xFF);//发送缓冲器0拓展标识符低位
	...
	MCP2515_WriteByte(RXF0SIDL,0xEB);//配置验收滤波寄存器n标准标识符低位(第3位为接收拓展标识符使能位)
	MCP2515_WriteByte(RXF0EID8,0xFF);//配置验收滤波寄存器n拓展标识符高位
	MCP2515_WriteByte(RXF0EID0,0xFF);//配置验收滤波寄存器n拓展标识符低位

	MCP2515_WriteByte(RXM0SIDH,0xFF);//配置验收屏蔽寄存器n标准标识符高位
	MCP2515_WriteByte(RXM0SIDL,0xE3);//配置验收屏蔽寄存器n标准标识符低位
	MCP2515_WriteByte(RXM0EID8,0xFF);//配置验收滤波寄存器n拓展标识符高位
	MCP2515_WriteByte(RXM0EID0,0xFF);//配置验收滤波寄存器n拓展标识符低位
